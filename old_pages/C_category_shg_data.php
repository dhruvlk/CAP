...
<table id="example" class="display nowrap table-bordered table-hover" cellspacing="0" width="100%">
      <thead>
        ...
        <tbody>
            <?php
			//$query="select s.enabler_id, s.shg_id,s.shg_group_name,s.total_member,s.shg_formation_date,s.shg_formation_date,s.shg_current_activity,s.shg_taluka,s.shg_village_name,s.shg_market_linkage,s.delete_status,s.financial_literacy,e.id,e.enabler_name from tbl_add_shg s,tbl_enabler e where s.financial_literacy='Intermediate' and s.org_id='".$_SESSION['org_id']."' and e.id=s.enabler_id and s.delete_status='Active'";

			$query="SELECT s.shg_id,s.shg_group_name,s. total_member,s.shg_formation_date,s.shg_current_activity,s.shg_taluka,s.shg_village_name,s1.creation_timestamp, s1.shg_category FROM tbl_add_shg s, shg_monthly_update s1 where s.shg_id = s1.shg_id and s.enabler_id='".$_GET['id']."' and s1.shg_category='C'and YEAR(creation_timestamp)= '".$_GET['year']."' and MONTH(creation_timestamp)='".$_GET['month']."'";
//echo $query;
