...
<table id="example" class="display nowrap table-bordered table-hover" cellspacing="0" width="100%">
      <thead>
        ...
        <tbody>
            <?php
			//$query="select tbl_add_shg.shg_group_name,tbl_add_shg.total_member,tbl_add_shg.shg_formation_date,tbl_add_shg.,tbl_add_shg.,tbl_add_shg., from tbl_add_shg,tbl_enabler where tbl_enabler.id='".$_GET['id']."' and tbl_add_shg.org_id='".$_SESSION['org_id']."' and  delete_status='Active'";
		//$query="select * from tbl_add_shg where org_id='".$_SESSION['org_id']."'  and delete_status='Active'";
		$query="select e.enabler_name,e.id,s.shg_id,s.shg_group_name,s.total_member,s.shg_formation_date,s.shg_current_activity,s.shg_taluka,s.shg_village_name,s.shg_market_linkage from tbl_enabler e,tbl_add_shg s where s.shg_status='APL' and s.org_id='".$_SESSION['org_id']."'  and s.delete_status='Active' and e.id=s.enabler_id" ;
