...
<table id="example" class="display nowrap table-bordered table-hover" cellspacing="0" width="100%">
      <thead>
		...
		$query="select s.enabler_id, s.shg_id,s.shg_group_name,s.total_member,s.shg_formation_date,s.shg_formation_date,s.shg_current_activity,s.shg_taluka,s.shg_village_name,s.shg_market_linkage,s.delete_status,s.functional_literacy,e.id,e.enabler_name from tbl_add_shg s,tbl_enabler e where s.functional_literacy='Yet_to_start' and s.org_id='".$_SESSION['org_id']."' and e.id=s.enabler_id and s.delete_status='Active'";
