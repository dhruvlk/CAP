...
<table id="example" class="display nowrap table-bordered table-hover" cellspacing="0" width="100%">
      <thead>
        ...
        <tbody>
			...
			//$query="SELECT s.shg_id,s.shg_group_name,s. total_member,s.shg_formation_date,s.shg_current_activity,s.shg_taluka,s.shg_village_name,s.enabler_id,s1.creation_timestamp, s1.shg_category FROM tbl_add_shg s,shg_monthly_update s1 where s.shg_id = s1.shg_id  and s.org='".$_SESSION['org_id']."' and s1.shg_category='A'and YEAR(creation_timestamp)= '".$_GET['year']."' and MONTH(creation_timestamp)='".$_GET['month']."'";
			$query="SELECT s.shg_id,s.shg_group_name,s. total_member,s.shg_formation_date,s.shg_current_activity,s.shg_taluka,s.shg_village_name,s1.creation_timestamp, s1.shg_category,e.enabler_name,e.id FROM tbl_add_shg s, shg_monthly_update s1,tbl_enabler e where s.shg_id = s1.shg_id  and s.enabler_id=e.id and s.org_id='".$_SESSION['org_id']."' and s1.shg_category='C'and YEAR(creation_timestamp)= '".$_GET['year']."' and MONTH(creation_timestamp)='".$_GET['month']."'";
//echo $query;
		//$query="select * from tbl_add_shg where financial_literacy = 'Yet_to_start' and enabler_id='".$_SESSION['enabler_id']."' and delete_status='Active'";
		...
		?>
			...
			<td>
				    ...
				    <a class="blue" href="view_shg_profile.php?shg_id=<?php echo $shg_id; ?>&id=<?php echo $id; ?>"   title="View">
						<i class="menu-icon fa fa-tachometer bigger-130"></i>
					</a>&nbsp;&nbsp;
					<?php /*?><a class="green" href="edit_shg.php?shg_id=<?php echo $shg_id; ?>" title="Edit">
