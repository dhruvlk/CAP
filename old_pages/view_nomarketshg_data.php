...
<table id="example" class="display nowrap table-bordered table-hover" cellspacing="0" width="100%">
      <thead>
		...
		//$query="select * from tbl_add_shg where  shg_market_linkage = 'No' and org_id='".$_SESSION['org_id']."'";
		$query="select s.enabler_id,s.shg_market_linkage,s.shg_id, s.shg_group_name,s.total_member,s.shg_formation_date,s.shg_formation_date,s.shg_current_activity,s.shg_taluka,s.shg_village_name,s.shg_market_linkage,s.delete_status,e.id,e.enabler_name from tbl_add_shg s,tbl_enabler e where  s.shg_market_linkage = 'No' and s.org_id='".$_SESSION['org_id']."' and e.id=s.enabler_id and s.delete_status='Active'";
