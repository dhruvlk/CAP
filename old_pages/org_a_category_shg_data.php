...
<table id="example" class="display nowrap table-bordered table-hover" cellspacing="0" width="100%">
      <thead>
        ...
        <tbody>
			...
			$query="SELECT s.shg_id,s.shg_group_name,s. total_member,s.shg_formation_date,s.shg_current_activity,s.shg_taluka,s.shg_village_name,s1.creation_timestamp, s1.shg_category,e.enabler_name,e.id FROM tbl_add_shg s, shg_monthly_update s1,tbl_enabler e where s.shg_id = s1.shg_id  and s.enabler_id=e.id and s.org_id='".$_SESSION['org_id']."' and s1.shg_category='A'and YEAR(creation_timestamp)= '".$_GET['year']."' and MONTH(creation_timestamp)='".$_GET['month']."'";
//echo $query;
